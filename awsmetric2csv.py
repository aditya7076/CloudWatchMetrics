...
with open(filename, 'wb') as csvfile:
    ...
    for instance in instances:
        ...
        result = utils.get_metric(resource, instance_id, period, days, 'CPUUtilization', 'AWS/EC2')
        item_list_arr = utils.process_metric(result)
        # write metrics to csv
        utils.write_to_csv(resource, csvwriter, instance, item_list_arr, 'CPUUtilization')

        result = utils.get_metric(resource, instance_id, period, days, 'MemoryUtilization', 'System/Linux')
        item_list_arr = utils.process_metric(result)
        # write metrics to csv
        utils.write_to_csv(resource, csvwriter, instance, item_list_arr, 'MemoryUtilization')
